...
class DumpTruck(Bibliomaniac):
	...
	def makeFilter(self, keyword, lastpage):
		self.bindPicture(keyword, lastpage)
	
		l_f = open(self.name + '_lightFilter.bin', 'wb') # l_f for light_file
		pickle.dump(self.routes, l_f)
		l_f.close()
		del l_f

		f_f = open(self.name + '_fineFilter.bin', 'wb') # f_f for fine_file
	...
	def judgeArticle(self):
		...
		while i < 5:
			...
			elif recentTweet[i] in self.fineFilter:
		...
		l_f = open(self.name + '_lightFilter.bin', 'wb')
